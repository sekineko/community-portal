<x-app-layout>
  <div class="w-full max-w-2xl mx-auto p-6 bg-white shadow-md md:my-4">
    <h2 class="text-xl text-primary font-bold mb-4 underline underline-offset-4">活動報告編集</h2>
    <form action="{{ route('reports.update', $report->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-4">
          ...
          <input type="date" id="date" name="date" value="{{ old('date', $report->date) }}" required
      ...
      <div class="mb-4">
          ...
          <textarea id="content" name="content" required rows="5"
              class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('content', $report->content) }}</textarea>
          @error('content')
            <div style="color: red;">{{ $message }}</div>
          @enderror
      </div>
      <div class="mb-4">
          <label for="image" class="block text-sm font-medium text-gray-700 mb-1">画像 (変更する場合のみ選択)</label>
