<x-app-layout>
    <div class="w-full max-w-2xl mx-auto p-6 bg-white shadow-md md:my-4">
        <h2 class="text-xl text-primary font-bold mb-4 underline underline-offset-4">行事予定編集</h2>
        <form action="{{ route('events.update', $event->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <div class="flex space-x-2">
                    <select id="start_hour" name="start_hour"
                        ...
                        <option value="" {{ old('start_hour', $event->start_time ? \Carbon\Carbon::parse($event->start_time)->format('H') : null) === null ? 'selected' : '' }}>-- 時 --</option>
                        @for ($hour = 0; $hour < 24; $hour++)
                            <option value="{{ $hour }}" {{ old('start_hour', $event->start_time ? (int) \Carbon\Carbon::parse($event->start_time)->format('H') : null) === $hour ? 'selected' : '' }}>
                    ...
                    <select id="start_minute" name="start_minute"
                        class="w-1/2 max-w-[200px] border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="" {{ old('start_minute', $event->start_time ? \Carbon\Carbon::parse($event->start_time)->format('i') : null) === null ? 'selected' : '' }}>-- 分 --</option>
                        @for ($minute = 0; $minute < 60; $minute += 5)
                            <option value="{{ $minute }}" {{ old('start_minute', $event->start_time ? (int) \Carbon\Carbon::parse($event->start_time)->format('i') : null) === $minute ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <div class="flex space-x-2">
                    <select id="end_hour" name="end_hour"
                        class="w-1/2 max-w-[200px] border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="" {{ old('end_hour', $event->end_time ? \Carbon\Carbon::parse($event->end_time)->format('H') : null) === null ? 'selected' : '' }}>-- 時 --</option>
                        @for ($hour = 0; $hour < 24; $hour++)
                            <option value="{{ $hour }}" {{ old('end_hour', $event->end_time ? (int) \Carbon\Carbon::parse($event->end_time)->format('H') : null) === $hour ? 'selected' : '' }}>
                                {{ str_pad($hour, 2, '0', STR_PAD_LEFT) }}
                            </option>
                        @endfor
                    </select>
                    <select id="end_minute" name="end_minute"
                        class="w-1/2 max-w-[200px] border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        <option value="" {{ old('end_minute', $event->end_time ? \Carbon\Carbon::parse($event->end_time)->format('i') : null) === null ? 'selected' : '' }}>-- 分 --</option>
                        @for ($minute = 0; $minute < 60; $minute += 5)
                            <option value="{{ $minute }}" {{ old('end_minute', $event->end_time ? (int) \Carbon\Carbon::parse($event->end_time)->format('i') : null) === $minute ? 'selected' : '' }}>
                                {{ str_pad($minute, 2, '0', STR_PAD_LEFT) }}
                            </option>
                        @endfor
                    </select>
            ...
            <div class="mb-4">
                ...
                <textarea id="content" name="content" rows="5"
                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('content', $event->content) }}</textarea>
                @error('content')
                    <div style="color: red;">{{ $message }}</div>
                @enderror
            </div>
